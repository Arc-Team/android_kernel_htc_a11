	...
	if (ts->cover_enable) {
		if (ts->package_id == 3528 )
		{
			if (ts->packrat_number >= SYNAPTICS_FW_35_COVER) {
				ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x54, CONTROL_BASE) + 14, ts->cover_setting[0]);
				if (ret < 0)
				{
				...
				ts->f54_ctrl89[6] = ts->cover_setting[1];
				ret = i2c_syn_write(ts->client, get_address_base(ts, 0x54, CONTROL_BASE) + 26, ts->f54_ctrl89, 8);
				if (ret < 0)
				{
				...
				ts->f12_ctrl10[1] = ts->cover_setting[2];
				ts->f12_ctrl10[0] = ts->cover_setting[7];
				...
				ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x12, CONTROL_BASE) + ts->ctrl_15_offset, ts->cover_setting[3]);
				if (ret < 0)
				{
				...
				ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x51, CONTROL_BASE) + 2, ts->cover_setting[4]);
				if (ret < 0)
				{
				...
				ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x51, CONTROL_BASE) + 3, ts->cover_setting[5]);
				if (ret < 0)
				{
				...
				ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x51, CONTROL_BASE) + 4, ts->cover_setting[6]);
				if (ret < 0)
				{
		...
		{
			if (ts->packrat_number >= SYNAPTICS_FW_35_COVER) {
				ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x54, CONTROL_BASE) + 14, ts->uncover_setting[0]);
				if (ret < 0)
				{
					i2c_syn_reset_handler(ts, ts->i2c_err_handler_en, "w:1", __func__);
					return;
				...
				ts->f54_ctrl89[6] = ts->uncover_setting[1];
				ret = i2c_syn_write(ts->client, get_address_base(ts, 0x54, CONTROL_BASE) + 26, ts->f54_ctrl89, 8);
				if (ret < 0)
				{
					i2c_syn_reset_handler(ts, ts->i2c_err_handler_en, "w:2", __func__);
				...
				ts->f12_ctrl10[1] = ts->uncover_setting[2];
				ts->f12_ctrl10[0] = ts->uncover_setting[7];
				ret = i2c_syn_write(ts->client, get_address_base(ts, 0x12, CONTROL_BASE) + ts->ctrl_10_offset, ts->f12_ctrl10, 2);
				if (ret < 0)
				{
					i2c_syn_reset_handler(ts, ts->i2c_err_handler_en, "w:3", __func__);
				...
				ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x12, CONTROL_BASE) + ts->ctrl_15_offset, ts->uncover_setting[3]);
				if (ret < 0)
				{
					i2c_syn_reset_handler(ts, ts->i2c_err_handler_en, "w:4", __func__);
					return;
				...
				ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x51, CONTROL_BASE) + 2, ts->uncover_setting[4]);
				if (ret < 0)
				{
					i2c_syn_reset_handler(ts, ts->i2c_err_handler_en, "w:5", __func__);
					return;
				...
				ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x51, CONTROL_BASE) + 3, ts->uncover_setting[5]);
				if (ret < 0)
				{
					i2c_syn_reset_handler(ts, ts->i2c_err_handler_en, "w:6", __func__);
					return;
...
static int syn_get_information(struct synaptics_ts_data *ts)
{
	...
	if (ts->package_id < 3400) {
		...
		if ((ts->package_id == 3528) & ts->cover_setting[0]) {
			if (ts->packrat_number >= SYNAPTICS_FW_35_COVER) {
				ret = i2c_syn_read(ts->client, get_address_base(ts, 0x54, CONTROL_BASE) + 14, data, 1);
				if (ret < 0)
					return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "r:2", __func__);
				...
				ts->uncover_setting[1] = data[6];

				ret = i2c_syn_read(ts->client, get_address_base(ts, 0x12, CONTROL_BASE) + ts->ctrl_10_offset, data, 2);
				if (ret < 0)
					return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "r:4", __func__);
				memcpy(ts->f12_ctrl10, data, 2);
				ts->uncover_setting[2] = data[1];
				ts->uncover_setting[7] = data[0];

				ret = i2c_syn_read(ts->client, get_address_base(ts, 0x12, CONTROL_BASE) + ts->ctrl_15_offset, data, 1);
				if (ret < 0)
...
static int synaptics_parse_config(struct synaptics_ts_data *ts, struct synaptics_i2c_rmi_platform_data *pdata)
{
	...
	while ((pp = of_get_next_child(node, pp))) {
		...
		{
			if (ts->packrat_number >= SYNAPTICS_FW_35_COVER) {
				if (of_property_read_u32_array(pp, "cover_setting", cover, 8) == 0) {
					...
					pr_info("[TP] cover setting parse failed\n");
	...
	if (i <= cnt) {
		...
		if (ts->packrat_number >= SYNAPTICS_FW_35_COVER) {
			memcpy(pdata->cover_setting, cfg_table[i].cover_setting, 8);
		} else {
...
static int synaptics_ts_suspend(struct device *dev)
{
	...
	if(ts->suspended)
	{
		pr_debug("[TP] %s: Already suspended. Skipped.\n", __func__);
		return 0;
	}
	else
	{
