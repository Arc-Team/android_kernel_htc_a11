...
static void dump_all(int more)
{
	...
	printk(KERN_INFO "[BATT][CHG] V=%d mV, I=%d mA, T=%d C, SoC=%d%%,id=%d mV,"
			...
			vin_min, host_mode, hsml_target_ma, batfet_no_lpm);
...
{
	unsigned int _chg_limit_current;
	...
	if (enable)
		_chg_limit_current = 800;
	else
		_chg_limit_current = 1000;

	qpnp_chg_iusbmax_set(the_chip, _chg_limit_current);   
